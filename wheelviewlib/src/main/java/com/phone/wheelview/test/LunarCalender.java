...
public class LunarCalender {
	 ...
	 * @param month 农历月
	 * @param monthDay 农历日
	 * @param isLeapMonth 该月是否是闰月 [url=home.php?mod=space&uid=7300]@return[/url] 返回农历日期对应的公历日期，year0, month1, day2.
	 */
	...
	private static final String NUMBER_NAME[] = { "零", "一", "二", "三", "四", "五", "六", "七", "八", "九" };

	private static final String MONTH_NAME[] = { "一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "冬月",
			"腊月" };
	private static final String DAY_NAME[] = { "初一", "初二", "初三", "初四", "初五", "初六", "初七", "初八", "初九", "初十", "十一", "十二",
			"十三", "十四", "十五", "十六", "十七", "十八", "十九", "廿", "廿一", "廿二", "廿三", "廿四", "廿五", "廿六", "廿七", "廿八", "廿九", "卅" };

	public static String numberToYearName(int number) {
		StringBuilder sb = new StringBuilder();
		sb.append(NUMBER_NAME[number / 1000] + NUMBER_NAME[number / 100 % 10] + NUMBER_NAME[number / 10 % 10]
				+ NUMBER_NAME[number % 10]);
		return sb.toString();
	}

	public static int yearNameToNumber(String name) {
		char thousandBit = name.charAt(0);
		char hundredBit = name.charAt(1);
		char tenBit = name.charAt(2);
		char bit = name.charAt(3);
		return nameToInt(thousandBit) * 1000 + nameToInt(hundredBit) * 100 + nameToInt(tenBit) * 10 + nameToInt(bit);
	}

	public static String numberToMonthName(int month, boolean isLeap) {
		return (isLeap ? "闰" : "") + MONTH_NAME[month - 1];
	}

	public static int monthNameToNumber(String monthName) {
		for (int i = 0; i < MONTH_NAME.length; i++) {
			if (monthName.startsWith("闰")) {
				monthName = monthName.substring(1);
			}
			if (MONTH_NAME[i].equals(monthName)) {
				return i + 1;
			}
		}
		return 0;
	...
	private static int nameToInt(char name) {
		for (int i = 0; i < NUMBER_NAME.length; i++) {
			if (NUMBER_NAME[i].equals(String.valueOf(name))) {
				return i;
			}
		}
		return 0;
	...
	/**
	 * 传回公历 year年month月的总天数
	 * 
	 * @param year
	 * @param month
	 * @return
	 */
	public static int daysInSolarMonth(int year, int month) {
