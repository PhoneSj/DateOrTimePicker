...
public class Lunar {
	...
	// ====== 传回农历 y年的总天数
	public static int getSumDaysOfYearForLunar(int y) {
		int i, sum = 348;
		for (i = 0x8000; i > 0x8; i >>= 1) {
			if ((lunarInfo[y - 1900] & i) != 0)
				sum += 1;
		}
		return (sum + getSumDaysOfLeapMonthForLunar(y));
	}

	// ====== 传回农历 y年闰月的天数
	public static int getSumDaysOfLeapMonthForLunar(int y) {
		if (whichMonthIsLeapForLunar(y) != 0) {
	...
	public static int whichMonthIsLeapForLunar(int y) {
		return (int) (lunarInfo[y - 1900] & 0xf);
	}

	// ====== 传回农历 y年m月的总天数
	public static int getSumDaysOfMonthFroLunar(int y, int m) {
	...
	// ====== 传回农历 y年的生肖
	public String getAnimalsForYear() {
		final String[] Animals = new String[] { "鼠", "牛", "虎", "兔", "龙", "蛇", "马", "羊", "猴", "鸡", "狗", "猪" };
		return Animals[(year - 4) % 12];
	}

	// ====== 传入 月日的offset 传回干支, 0=甲子
	private static String getCyclicalm(int num) {
	...
	public String cyclical() {
		...
		return (getCyclicalm(num));
	}

	/** */
	/**
	 * * 传出y年m月d日对应的农历. * yearCyl3:农历年与1864的相差数 ? * monCyl4:从1900年1月31日以来,闰月数 * dayCyl5:与1900年1月31日相差的天数,再加40 ? * @param
	 * cal * @return
	 */
	public Lunar(Calendar cal) {
		...
		for (iYear = 1900; iYear < 2100 && offset > 0; iYear++) {
			daysOfYear = getSumDaysOfYearForLunar(iYear);
			//			System.out.println(" year = " + iYear + " daysOfYear = " + daysOfYear);
			offset -= daysOfYear;
			monCyl += 12;
		}
		if (offset < 0) {
		...
		year = iYear;
		yearCyl = iYear - 1864;
		leapMonth = whichMonthIsLeapForLunar(iYear); // 闰哪个月,1-12
		leap = false;
		...
		for (iMonth = 1; iMonth < 13 && offset > 0; iMonth++) {
			...
			if (leapMonth > 0 && iMonth == (leapMonth + 1) && !leap) {
				--iMonth;
				leap = true;
				daysOfMonth = getSumDaysOfLeapMonthForLunar(year);
			} else
				daysOfMonth = getSumDaysOfMonthFroLunar(year, iMonth);
			offset -= daysOfMonth;
			// 解除闰月
	...
	public static void main(String[] args) throws ParseException {
		...
		today.setTime(chineseDateFormat.parse("2016年12月31日"));
